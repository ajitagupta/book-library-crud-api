...
Spring Boot uses additional annotations. This includes `ComponentScan` to extend the packages to be searched for beans. For RESTful services, it uses the `RestController` annotation along with subordinate annotations like `Get-/Post-/Put-/DeleteMapping` to make it possible to handle different RESTful service requests. Further, Spring uses **Aspect-Oriented Programming (AOP)** to separate individual concerns in different layer packages: one for model, controller, service, respositories, exceptions, and response.
